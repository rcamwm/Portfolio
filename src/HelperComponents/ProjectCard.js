...
function ProjectCard(props) {
  const {title, description, technologies, githubLink, liveLink, reportLink, techFilters} = props;
      ...
      <div className="flex rounded-md h-full max-w-sm text-white border bg-card-bg bg-opacity-70 py-2 px-4 flex-col">
        ...
        <div className="mt-auto w-fill">
          <footer className={"mt-4 text-center flex flex-wrap justify-center"}>
            ...
            {reportLink === "" ? null : (
              <a className="mx-1 px-2 py-1 text-xs flex font-jetbrains hover:scale-105 border rounded-md" href={reportLink} target="_blank" rel="noreferrer">Read More</a>
            )}
          </footer>
        </div>
      </div>
